...
exports.productById=(req,res,next,id_val)=>{
    ...
    Product.findOne({'_id':id_val}).populate("category").exec((err,product)=>{ 
        if(err || !product){    
    ...
    form.parse(req,(err,fields,files)=>{
        ...
        if(files.photo){ 
            if(files.photo.size>1000000){ 
                console.log(files.photo)                          //checking image size
                return res.status(400).json({ 
...
exports.update=(req,res)=>{
    ...
    form.parse(req,(err,fields,files)=>{
        ...
        if(files.photo.size>1000000){                           //checking image size
            return res.status(400).json({ 
                error : "image size must be less than 1MB! "
            })
        }
...
exports.relatedProducts=(req,res)=>{
    ...
    Product.find({_id:{$ne:req.product._id},category:req.product.category}).select('-photo').populate('category').limit(limit).exec((err,results)=>{
        if(err){
            return res.status(400).json({
                error:err
            })
